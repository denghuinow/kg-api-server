...
| `nodes[].properties` | object \| null | 节点属性对象，`include_properties=false` 时为 `null`。不包含 `entity_label`、`name`、`embeddings`、`kg_version` 等内部字段 |
...
| `edges[].properties` | object \| null | 边属性对象，`include_properties=false` 时为 `null`。不包含 `predicate`（已在 `type` 字段中）、`embeddings`、`kg_version` 等内部字段。可能包含 `atomic_facts`、`t_obs`、`t_start`、`t_end` 等业务字段 |
